...
function getRandomRecipe() {
	let randomInt = Math.floor(Math.random() * recipes.length)
...
function filterRecipes(text) {
  return recipes.filter(recipe => searchList(recipe, text));
}

function searchList(recipe, text) {
...
search.addEventListener("click", function () {
  let searchText = document.body.querySelector(".search_text").value.toLowerCase();
  if (searchText !== "") {
    renderRecipes(filterRecipes(searchText));
  }
});
